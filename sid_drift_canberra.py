...
#next test: stp=50, factor=1, template=90, much smaller sub-sample around Lance:
#lon_diff = 10
#lat_diff = 1
...
#outpath_drift = '/src/data/stp1_afternoon/'
#outpath = '/src/plots/stp1_afternoon/'

outpath_drift = '../sidrift/data/40m_parcels/'
outpath = outpath_drift


# ==== ICE DRIFT RETRIEVAL ====
#inpath = '/src/Sentinel1/'
inpath = '../sidrift/data/Sentinel1_handpick24h_afternoon/'
...
metfile = '/src/data/10minute_nounits.csv'
...
for i in range(0,len(fl)):    
    ...
    if mode != 'all':
        ...
        #24h: expected time difference: 22-24 h 
            ...
            match = np.argmin(abs(dt_diff - timedelta(hours=24)))
    ...
    ##prepare for PM
    #sid = SeaIceDrift(f1, f2, factor=factor)
    ...
    ## this is a regular grid, so steps can be regular
    ...
    #print(lon1pm.shape)
    #print('Ready to process drift')
    
    
    sid = SeaIceDrift(f1, f2, factor=factor)    #enough for feature tracking
    ...
    try:
        ...
        upm, vpm, apm, rpm, hpm, lon2pm, lat2pm = sid.get_drift_PM(lon1pm, lat1pm, lon1ft, lat1ft, lon2ft, lat2ft, srs=srs, img_size=template)
    except:
        continue

    #calculate velocity from displacements
    #WARNING! Check if this is still necessary!!!!
    upm = upm/timediff
    vpm = vpm/timediff
    ...
    ##this last part often doesnt work - the lon2pm is too short
    #print(lonlat_shape)
    #print(hpm.shape)
    #print(hpm2.shape)
    #print(hpm2[near_lance_pix].shape)
    #print(lon2pm2.shape)
    #print(lon2pm.shape)
    #print(lon1pm.shape)
    #print(lon2pm2[near_lance_pix].shape)
    
    #temp = np.zeros_like(lon1pm) + np.nan
    #temp[:len(lon2pm)] = lon2pm    
    #lon2pm2[near_lance_pix] = temp
    
    #temp = np.zeros_like(lat1pm) + np.nan
    #temp[:len(lat2pm)] = lat2pm    
    #lat2pm2[near_lance_pix] = temp    
    ...
    #clean up
    del sid, uft, vft, lon1ft, lat1ft, lon2ft, lat2ft,lon1pm, lat1pm,upm, vpm, apm, rpm, hpm, lon2pm, lat2pm
    del upm2, vpm2, apm2, rpm2, hpm2, lon1pm2, lat1pm2
