...
#same projection as in pattern matching part of the sea ice drift algorithm
...
area_def = AreaDefinition(area_id, description, proj_id, proj_dict, 300, 1000, area_extent)
...
for day in days:
    ...
    #try:
    print(inpath+'Tile_'+str(step)+'_'+str(factor)+'_'+day+'*'+file_name_end+'.npz')
    fname=sorted(glob(inpath+'Tile_'+str(step)+'_'+str(factor)+'_'+day+'*'+file_name_end+'.npz'))[0]
    print(fname)
    ...
    dt1s = container['dt1s']
    dt2s = container['dt2s']
    diffs = container['diffs']
    
    print('Size of input matrix:', xs.size)
    exit()
