...
outname_td = inpath+'td_'+reg+file_name_end+'_strict30_subsample.npz'
powerlaw = 'powerlaw_test_'+reg+file_name_end+'_strict30_subsample.png'
...
for fname in fnames:
    ...
    dst_base = int(fname.split('_')[3])
    #dst=800
    
    #threshold
    #dummy_ls_all, dummy_td_all, dummy_max_td_all
    threshold_file = glob(inpath+'dummy_'+reg+'_'+date+'*c'+rname+'.csv')[0]    #center tile is OK for plotting...
    ...
    #start a new multiplot for scales
    ...
    idx=1   #subplot index
    for j in stp[:-1]:  #no data for last stp  
        print('step: ',j)
        dst=dst_base*j
    
        fname2 = fname.split('stp1')[0]+'stp'+str(j)+'_tiled.npz'
        print(fname2)
        
    
        #load data
        container = np.load(fname2, allow_pickle=True)
        ...
        #div = np.ma.masked_invalid(div).compressed()
        #shr = np.ma.masked_invalid(shr).compressed()
        #area = np.ma.masked_invalid(area).compressed()
        #tripts = np.ma.masked_invalid(tripts).compressed()
        ...
        #td = np.ma.masked_invalid(td).compressed()
        #ls = np.ma.masked_invalid(ls).compressed()
    ...
    plt.show()
    multiplot = 'multiplot_'+file_name_end+'_'+reg+'_'+date+'_'+file_name_end+'_subsample.png'
...
#write data for scatter plots
np.savez(outname_td, ls_lists=ls_lists, td_lists=td_lists, tls_list=tls_list)
