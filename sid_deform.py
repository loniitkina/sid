...
lscale = 'full'
print(lscale)
#steps between the image pixels
...
for j in stp:
    ...
    for i in fl:
        ...
        print(i)
        u = np.load(i)[::j, ::j] 
        v = np.load(i.split('_upm')[0]+'_vpm.npy')[::j, ::j]           #in m/s???
        rpm = np.load(i.split('_upm')[0]+'_rpm.npy')[::j, ::j]        #cross-correlation matrix
        hpm = np.load(i.split('_upm')[0]+'_hpm.npy')[::j, ::j]        #hessian
        lat = np.load(i.split('_upm')[0]+'_lat1pm.npy')[::j, ::j] 
        lon = np.load(i.split('_upm')[0]+'_lon1pm.npy')[::j, ::j] 
        ...
        #cut out region
        radius = 15000
        mask = (x<xls-radius) | (x>xls+radius) | (y<yls-radius) | (y>yls+radius)
        ...
        #exit()
            ...
            #deform = div*1e6
            ##print(deform)
            #outname = 'map_div_'+reg+str(j)+'_'+date1
            ...
            ##deform = shr*1e6
            ##outname = 'map_shr_'+str(j)+'_'+date1
            ...
            ###deform = hpm
            ##print(deform)
            ##outname = 'map_rpm_'+str(j)+'_'+date1
            ##label = r'Error'
            ###interval = [0, 20]
            ##interval = [0, 1]
            ##cmap=plt.cm.Reds
